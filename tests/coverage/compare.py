...
def parse_args() -> tuple[float, Path]:
    ...
    parser.add_argument("reference_directory", help="Directory containing historic coverage data.")
...
def check_significant_difference(current_coverage: float, historic_coverage: float) -> bool:
    """
    ...
    if absolute_loss < 0:
        print(
            f"PASS: Coverage increased by {-absolute_loss}% from {historic_coverage:.2f}% to {current_coverage:.2f}%."
        )
        return False
    ...
    print(
        f"PASS: Coverage reduced slightly by {absolute_loss}% from {historic_coverage:.2f}% to {current_coverage:.2f}%."
    )
    return False
