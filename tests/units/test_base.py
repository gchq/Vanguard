...
class InputTests(VanguardTestCase):
    ...
    def test_unsqueeze_y(self) -> None:
        ...
        gp = GPController(train_x=self.DATASET.train_x, train_y=squeezed_train_y, kernel_class=PeriodicRBFKernel,
                          mean_class=ConstantMean, y_std=self.DATASET.train_y_std,
    ...
    def test_error_handling_of_higher_rank_features(self) -> None:
        ...
        gp = GaussianGPController(train_x=train_x_mean, train_y=self.DATASET.train_y, kernel_class=PeriodicRBFKernel,
                                  y_std=self.DATASET.train_y_std)
        shape_rx = rf"\({shape[0]}, {shape[1]}, {shape[2]}\)"
        expected_regex = rf"Input data looks like it might not be rank-1, shape={shape_rx}\. If your features are "\
...
class NLLTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # NLL on test set
        self.sklearn_nll = self.predictive_nll(z.flatten(), z_std**2, self.noise_variance,
    ...
    def test_gpytorch_nll(self) -> None:
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            std = (upper - observed_pred.mean).detach().cpu().numpy() / 2.
