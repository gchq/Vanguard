...
class StandardiseModuleTests(unittest.TestCase):
    ...
    def _test_standardised_module_gives_same_results_as_plain_module_on_different_data(self, base_module,
                                                                                       scaled_module):
    ...
    def _test_standardised_module_gives_different_results_to_plain_module_on_raw_data(self, base_module,
                                                                                      scaled_module):
        standard_output = self._to_numpy(scaled_module(self.data))
    ...
    def _test_standardised_module_gives_different_results_to_plain_module_on_standardised_data(self, base_module,
                                                                                               scaled_module):
        standard_output = self._to_numpy(scaled_module(self.standardised_data))
        plain_output = self._to_numpy(base_module(self.standardised_data))
        np.testing.assert_array_less(-np.abs(standard_output-plain_output).mean(), -1e-6)

    def _test_standardised_module_gives_different_results_to_plain_module_on_different_data_wrong_way(self, base_module,
                                                                                                      scaled_module):
        standard_output = self._to_numpy(scaled_module(self.standardised_data))
        plain_output = self._to_numpy(base_module(self.data))
        np.testing.assert_array_less(-np.abs(standard_output-plain_output).mean(), -1e-6)
    ...
    def test_standardised_mean_gives_same_results_as_plain_mean_on_different_data(self):
        self._test_standardised_module_gives_same_results_as_plain_module_on_different_data(
            self.base_mean, self.scaled_mean)

    def test_standardised_mean_gives_different_results_to_plain_mean_on_raw_data(self):
        self._test_standardised_module_gives_different_results_to_plain_module_on_raw_data(
            self.base_mean, self.scaled_mean)

    def test_standardised_mean_gives_different_results_to_plain_mean_on_standardised_data(self):
        self._test_standardised_module_gives_different_results_to_plain_module_on_standardised_data(
            self.base_mean, self.scaled_mean)

    def test_standardised_mean_gives_different_results_to_plain_mean_on_different_data_wrong_way(self):
        self._test_standardised_module_gives_different_results_to_plain_module_on_different_data_wrong_way(
            self.base_mean, self.scaled_mean)

    def test_standardised_kernel_gives_same_results_as_plain_kernel_on_different_data(self):
        self._test_standardised_module_gives_same_results_as_plain_module_on_different_data(
            self.base_kernel, self.scaled_kernel)

    def test_standardised_kernel_gives_different_results_to_plain_kernel_on_raw_data(self):
        self._test_standardised_module_gives_different_results_to_plain_module_on_raw_data(
            self.base_kernel, self.scaled_kernel)

    def test_standardised_kernel_gives_different_results_to_plain_kernel_on_standardised_data(self):
        self._test_standardised_module_gives_different_results_to_plain_module_on_standardised_data(
            self.base_kernel, self.scaled_kernel)

    def test_standardised_kernel_gives_different_results_to_plain_kernel_on_different_data_wrong_way(self):
        self._test_standardised_module_gives_different_results_to_plain_module_on_different_data_wrong_way(
            self.base_kernel, self.scaled_kernel)
...
class DisableStandardiseModuleTests(StandardiseModuleTests):
    ...
    def setUp(self):
        ...
        torch.manual_seed(self.seed)  # reset the seed to reproduce the random parameters
        gp = DisableStandardScalingController(train_x=self.data, train_y=np.random.randn(self.data.shape[0]),
                                              y_std=0, kernel_class=RBFKernel, mean_class=type(self.base_mean))
