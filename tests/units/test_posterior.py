...
class BasicTests(unittest.TestCase):
    ...
    def test_1_dim_mean_confidence_interval(self):
    ...
    def test_2_dim_mean_confidence_interval(self):
        covar = torch.diag(self.std ** 2)
    ...
    def test_2_task_confidence_interval(self):
    ...
    def test_1_dim_mean_log_probability_size(self):
        covar = torch.diag(self.std**2)
        posterior = Posterior.from_mean_and_covariance(self.mean, covar)
        log_prob = posterior.log_probability(torch.randn(*self.mean.shape))
        self.assertLess(log_prob, 0)

    def test_1_dim_mean_log_probability_order(self):
        covar = torch.diag(self.std**2)
        posterior = Posterior.from_mean_and_covariance(self.mean, covar)
