"""
All warp functions should subclass this :py:class:`WarpFunction` class.
"""
import copy
...
class WarpFunction(gpytorch.Module):
    ...
    Subclasses must implement the :py:meth:`forward` and :py:meth:`inverse` methods. Optionally, the
    :py:meth:`deriv` method can be implemented. If not, then it defaults to autograd, which is significantly slower.
    ...
    def compose_with_self(self, n):
        ...
        .. warning::
            ...
            :py:class:`~vanguard.warps.SetWarp` decorator, the warp function
    ...
    def compose(self, other):
        ...
        .. note::
            For convenience, it is often easier to use the ``@`` operator in place of :py:meth:`compose`.
    ...
    def freeze(self):
        ...
        .. note::
            We override :py:attr:`torch.nn.Module.parameters` to return an empty generator, so no amount of
    ...
    def _combined_parameters(self):
        ...
        Used in composition warps to override the default :py:attr:`torch.nn.Module.parameters` so that
